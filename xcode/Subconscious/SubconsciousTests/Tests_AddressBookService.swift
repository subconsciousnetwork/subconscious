...
final class Tests_AddressBookService: XCTestCase {
    ...
    func testExpectedNameAndAliases() async throws {
        let tmp = try TestUtilities.createTmpDir()
        let data = try await TestUtilities.createDataServiceEnvironment(tmp: tmp)
        let addressBook = data.addressBook
        ...
        let did = Did("did:key:123")!
        let petname = Petname("ziggy")!
        let petname2 = Petname("ziggy-2")!
        
        try await addressBook.followUser(did: did, petname: petname)
        try await addressBook.followUser(did: did, petname: petname2)
        
        let a = await addressBook.followingStatus(did: did, expectedName: petname.leaf)
        XCTAssertEqual(a, UserProfileFollowStatus.following(petname.leaf))
        
        let b = await addressBook.followingStatus(did: did, expectedName: petname2.leaf)
        XCTAssertEqual(b, UserProfileFollowStatus.following(petname2.leaf))
        
        let aliases = try await addressBook.listAliases(did: did)
        XCTAssertTrue(aliases.count == 2)
        XCTAssertEqual(aliases, [petname, petname2])
    }
    ...
    func testIsFollowingUserAndHasEntryFor() async throws {
        ...
        let a = await addressBook.followingStatus(did: did, expectedName: nil)
        ...
        let c = await addressBook.followingStatus(did: did, expectedName: nil)
        let d = await addressBook.hasEntryForPetname(petname: petname)
        XCTAssertTrue(c.isFollowing)
        XCTAssertTrue(d)
        
        let _ = try await addressBook.unfollowUser(petname: petname)
        
        let e = await addressBook.followingStatus(did: did, expectedName: nil)
